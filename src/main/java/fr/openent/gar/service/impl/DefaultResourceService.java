            ...
            if (event.isRight()) {
                ...
                if (results.size() > 0) {
                    ...
                    garRequest(httpClient, resourcesUri, garHostNoProtocol, uai, structureName)
                        ...
                        .onFailure(err -> {
                            log.error("[DefaultResourceService@get] An error occurred when fetching structure UAI");
         ...
         .onSuccess(response -> {
             if (response.statusCode() != 200) {
                 ...
                 response.exceptionHandler(throwable -> {
                         log.error("[DefaultResourceService@get] failed to get GAR response: " + throwable.getMessage());
                         promise.fail(throwable.getMessage());
                 });
             }
         })
         .onFailure(err -> {
             log.error("[DefaultResourceService@get] failed to request :" + err.getCause().getMessage());
